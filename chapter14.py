...
#  ？ 重复0次或者一次    | 选择    {} 定义量词   [] 定义字符类
#  () 分组   ^ 取反或者一行的开始    $ 匹配一行的结束
# encoding='utf-8'
import re
# p1= r'\w+@zhijieketang\.com'
# r的作用为：在原始字符串里，所有的字符都是直接按照字面的意思来使用，没有转义特殊或不能打印的字符。
# #p1去除了转义字符的的等价的普通字符串的表述：'\\w+@zhijieketang\\.com', why
# p2= r'^\w+@zhijieketang\.com$'   #开始^且尾部$匹配,
# text="Tony‘s email is tony_588@zhijieketang.com"
# m=re.search(p1,text)
# print(m)
# m=re.search(p2,text)    #只要存在就可以返回True
# print(m)
...
# m = re.search(r'\d{1,8}?','87654321')  #  此时只会匹配8, 一位
# print(m)
...
# print(m)           # 返回地址
# print(m.groups())  # 返回两组('010', '98765432')
'''访问分组除了可以通过组编号，也可以通过组名访问
前提是要在正则表达式中为组命名。组命名通过在组开头添加“？P＜分组名〉”实现。'''
# import re
...
# print(m)    #这里返回None，因为反向引用使得前后必须一致
'''使用非捕获分组，当并不想引用子表达式的匹配结果，不想捕获严格的匹配结果，只是将小括号作为一个整体进行匹配的时候。
使用方法加‘？’  '''
# import re
# s='img1.jpg, ing2.jpg, img3.jpg'
# p=r'\w+(\.jpg)'
# mlist=re.findall(p,s)     #捕获分组
# print(mlist)
# p=r'\w+(?:\.jpg)'         # 非捕获分组将整个括号内的分组，即？的部分也包括
...
4. 多行模式，使得^和$匹配任意一行的开始与结束。默认为整个字符串的开始和结束。
...
p=  """(java)  #正则表达式原本是(java).*(python）
       ...
       (python)
    """
regex=re.compile(p, re.I|re.VERBOSE)    #当需要设置多编译标志时， 编译标志之间需要位或运算符‘|’ 。
