...
'''
# time和datetime模块
# time 为底层的C接口，严重依赖于硬件平台，而datetime进行了封装，主要有以下的几个类
'''
datatime：时间和日期
date: 日期
time：时间
...
'''类似的函数如下   #UTC 协调世界时间,它以原子时为基础，是时刻上尽量接近世界时的一种时间计量系统
...
# 使用strftime()和strptime()中都有一个格式化参数format,用来控制日期时间的格式,使用为%,.%y等等
# import datetime
...
# bj_dt=utc_dt.astimezone(bj_dt)       # 重新定义时区。
...
'''
# import logging
# logging.basicConfig(level=logging.ERROR)   # logging之中设置日志级别为 ERROR# 改变日志级别为DEBUG ，所有日志函数信息都能输出，
# logging.debug("这是debug信息")     #  输出其与其上级的所有警告
# logging.info("这是info信息")
# logging.warning("这是Warning信息")
# logging.error("这是ERROR信息")
# logging.critical("这是Critical信息")
# root 关键字，这说明进行日志输出的对象是root日志器（logger），也
# 可以使用getLogger（）函数创建自己的日志器对象，
