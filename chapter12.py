...
def read_date(in_data):
    try:
        date=dt.datetime.strptime(in_data,'%Y-%m-%d')  #  作用为将a
        return date
    except ValueError as e:   # 获得异常对象，赋值给e值
        print('处理ValueError异常')
        tb.print_exc()  #  打印堆栈信息
        print(e)  # 这里会报错。unconverted data remains: -00，即错误的地方
    finally:
        print("final处理")
str_data = '2018-8-18-00'
...
# 为了自动管理资源，可以在try中使用with as语句，至此而不需要使用finally代码。自动释放资源
