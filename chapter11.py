...
'''
在Python 中一切数据类型都是面向对象的，即类与实例.面向对象的封装与真实世界的目的是一样的。
...
对象的生命周期包括：创建、使用和销毁。销毁是自动的
'''
# class Animal(): # 声明
#     pass        # 不作为
# cat=Animal()    # 实例化
'''
# 类的成员：成员变量(attributer)、成员方法和属性(property)
成员变量又分为特有的实例变量和共有的类变量;
成员方法又分为实例方法、类方法和静态方法。
使用setter()和getter()函数对attributer()进行设定和操作。
'''
# class Animal(): # 声明
#     name='Animal'    # 类变量，共有
#     def __init__(self,age,sex,weight):   # 私有变量
...
# # print(cat.sex)   # 访问方法.
# # print(cat.name)
# cat.add='change_add'   #这个实例变量无法通过类中的方法访问。但可以实例访问
...
class Animal(): # 声明
    ...
    def __init__(self,age,sex,weight=0):   # 私有变量
        ...
        self.age=age
        self.sex=sex
        self.__weight=weight   #Python 的封装性使用命名实现，私有变量是__开头即可
    # @classmethod   #装饰器Decorators用于calssmate声明类方法
    ...
    #     self.__weight+=0.05
    #     print('eat')
    # def __run(self):   #私有方法，改名类似。
    #     self.__weight-=0.04
    #     print('run')
    # @staticmethod   #静态方法只是为了提供一个基于类名的命名空间
    # def static(amt):  #不指明cls和self,只传递参数。
    #     return Animal.NameReturn(amt)
    @property   #用于getter
    def weight(self):      #定义属性时应该先定义getter 访问器， 再定义setter 访问器
...
cat1.set_weight(123)
# cat1._Animal__run()   # 非常规调用这些实例方法， 注意其中不需要传入self 参数。
...
#  在严格意义上的面向对象设计中， 一个类不应该公有实例成员变量的，应该全为私有，通过公有的setter与getter 访问器访问。
...
# 用isinstance(object, classinfo)函数可以检查其实例是否为某一个类。
# 例如：
...
它是所有类的“祖先”，object有很多方法，这意味着所有的类通用。两个可以重写的典型： __str__() ： 返回该对象的字符串表:为了日志输出等处理方便，所有的对象都可以输出自己的描述信息
