...
# 特点：简单易学，面向对象，解释性，免费开源，胶水语言，丰富的库，强制缩进的规范代码，函数编程，提供了函数式编程的支持，如函数类
# 型、Lambda 表达式、高阶函数和匿名函数等。动态类型。
# 应用：高级语言，无法进行硬件访问(类似于Java)，应用于应用开发，web应用，自动化，科学计算和数据可视化，网络爬虫（urllib，Selenium BeautifulSoup 等，还有网络爬虫框架scrapy）。
# 人工智能和大数据（Hadoop，Spark），游戏开发（Pygame 、Pyglet 和Cocos2d）
...
# 交互式时，可以直接在Anaconda里面输入python即可。 用windows 的命令可能不行，除非另外安装了IDLE。
...
# python共计33个关键词，False,None,True 大写，其余全部小写。
# coding=utf-8 用于设定编码格式，必须放在前两行才有效。
a=b=c=d=142  #支持链式赋值
f=0
...
# import re 每一句道路语句只能道路一个模块，但from re impoet 1,2,3,可以接上多个元素
# 导入语句总是放在文件顶部，位于模块注释和文档注释之后，模块全局变量和常量之前。导入语句应该按照从通用到特殊的顺序分组， 顺序是： 标准库→ 第三方库→ 自己模块
# 有可能标准库中的模块有自己的顺序。
# 过长的语句可以直接在括号中断开，而无需使用换行符’\‘,有时候也直接人为加入货号去点换行符
...
Python 中布尔类型为bool, bool 是int 的子类，它只有两个值： True 和False 。通过函数bool()转化为bool类型。
  ...
  字符串：python内部的字符串是有顺序的，从左到右， 索引从0 开路理始依次递增。Python 中字符串类型是str。
三种字符串类型：1. 普通的，使用（单/双）引号生成的；2. 原始字符串：在普通之前加上r，作用为特殊字符不需要转换而按照原来的面目显现。3.
                 长字符串：包含了换行缩进等排版字符，可以使用三重单引号’‘’或三重双引号“”“包裹起来，这就是长字符串。
Python 中的字符采用 Unicode 编码， 所以字符串可以包含中文等亚洲字符
  如果想在字符串中包含一些特殊的字符，例如换行符、制表符等， 在普通字符串中则需要转义，前面要加上反斜杠“＼飞这称为字符转义。
'''
# e.g.转义
print('Hello \n world')  #换行
...
# 使用format() 进行格式化输入，即实现替换功能。注意那个format前面的点，例子如下
name1 = 'Klaus'
name2= 'Mike'
position='Doctor'
s='{0}的职位是{1}'.format(name1,position)
print(s)
s='{p}是{k}'.format(p=name2,k=position)
print(s)
a='Ab,{0}'
s=a.format(5)    #注意这是可以分离写的。
print(s)
...
# 注释：函数与方法的区别是，方法是定义在类中的函数，在类的外部调用时需要通过类或对象调用。而(顶层)函数不在类中，直接可以调用。
# 字符和数值的转换,例子如下：
int('AC',16)  # 指定十六进制否则默认十进制AD不存在会报错的
...
c+=10
#  其余的测试运算符如下 ，is, is not, 判定是否相同，就是==与!=；一个关于类的较为复杂的例子如下
class Person:
    def __init__(self, name, age):
        self.name=name
        self.age=age
    def __eq__(self, other):  #重新定义等号的测定，使得后面为True，其余的符号操作类似
...
# if 和 else 的使用。例子如下
'''
import sys    # run chapter9.py 98 在下面的 Python 控制台中直接调用即可。
...
if a<1:
    print('a<1')
elif a>1 & a<5:    #注意这里只会判定一个，必须使用and替代。类似的还有，not and or 一共三个
    print('a>1 and a<5')
else:
    print('a=10')
...
# while 语句如下
a=1
while a<20:    #注意其缺陷：while 循环条件语句中只能写一个表达式， 而且是一个布尔型表达
    a=a+8
else:
    print(a)
